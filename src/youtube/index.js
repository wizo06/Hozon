...
const profile = require(path.join(process.cwd(), 'src/youtube/profile.js'));
const post = require(path.join(process.cwd(), 'src/youtube/post.js'));
...
.on('close', async () => {
  ...
  for (username of arrOfUsernames) {
    ...
    fs.mkdirSync(path.join(process.cwd(), `archives/youtube/${username}`), { recursive: true });
